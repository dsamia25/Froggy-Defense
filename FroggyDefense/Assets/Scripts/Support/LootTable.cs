...
namespace Support
{
    ...
    {
        private Dictionary<T, int>
            _table = null; // Table linking each item to its roll weight. The roll weight is how much of a chance that item has to be rolled as its percent contribution to the total weight of all items.
        ...
        public void LootTableTest(int rolls)
        {
            ...
            {
                review += "item: " + result.Key.ToString() + ", weight: " + _table[result.Key] + "(" +
                          (float)(100.0f * _table[result.Key] / _totalWeight) + "%), rolled " + result.Value +
                          " times (" + (float)(100.0f * result.Value / rolls) + "%)\n";
            }
